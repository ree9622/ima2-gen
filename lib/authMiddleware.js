...
export function authMiddleware(req, res, next) {
  // Internal loopback jobs are enqueued by an already-authenticated request,
  // then replayed by the server worker after the browser request is gone.
  // server.js verifies a per-process random token before setting this flag.
  if (req.internalQueueAuth && req.authUser) return next();
