...
// independent and written via atomic rename (no locking, no torn JSON).
...
export async function appendBatchEntry({
  ...
  await atomicWriteJson(file, enriched);
}
...
// Mark a batch as completed: stamps _meta.completedAt + _meta.summary,
// returns the freshly-computed summary so the caller can log it. Idempotent
// — calling close twice overwrites with the latest snapshot, which is what
// you want when entries are still trickling in after a client-side timeout.
export async function closeBatch({ generatedDir, batchId, billingAfter = null }) {
  if (!isValidBatchId(batchId)) return null;
  const dir = batchDir(generatedDir, batchId);
  ...
  try {
    await atomicWriteJson(join(dir, "_meta.json"), meta);
  } catch {
    // best-effort — read path will recompute summary regardless
  }
  return { meta, summary, entries: result.entries };
}
