// 좌측 패널에서 켜고 끌 수 있는 층. 여기에는 "브리프에 어느 정도 재량을
// 주는가"만 담는다. 출력 채널 계약과 브리프 읽기 규칙은 끌 수 없는
// lib/developerPrompts.js 쪽에 있다 — 이 토글이 꺼졌을 때 실패율이 25%에서
// 63%로 뛴 원인이 계약까지 함께 사라지는 구조였기 때문이다(2026-07-30 실측).
export const DEFAULT_PROMPT_INJECTION = [
  ...
  // 2026-07-30: 최근 EMPTY_RESPONSE 거절문 38건이 사실상 전부 "피팅룸 거울
  // 셀카 + 수영복" 조합이었다. 기존 allowlist 에 그 장면 유형이 빠져 있어
  // 같은 계열의 일상 착장 씬을 계속 특별 취급했다. 긍정 프레이밍으로
  // 장면 유형만 추가한다(fff1fcd 선례).
