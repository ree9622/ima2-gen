...
//
//   [퀄리티]   단 한 줄 "1:1 비율" — 비율만. '8k', '극사실적',
//             'masterpiece', 'best quality' 등은 사용자 명시 요청 시에만.
//             '8k' 는 2026-07-30에 제거했다. OpenAI 공식 프롬프팅 가이드가
//             "8k / masterpiece 류 키워드 스팸은 모델이 읽지 않는다"고
//             명시한다(gpt-image 계열은 2023 Midjourney 태그가 아니라
//             서술형 자연어를 읽는다). 비율은 남긴다 — 도구의 size 인자가
//             그대로 지켜지지 않아 문장으로 밀어넣는 쪽이 실효가 있다.
//
...
const COMMON_RULES = [
  ...
  "[퀄리티] / [Quality] — 기본은 단 한 줄 `- 1:1 비율` (영문 `- 1:1 ratio`), 비율만 적는다. FORBIDDEN by default: '8k', '4k', '극사실적', 'photorealistic', 'hyper-realistic', 'masterpiece', 'best quality', 'no watermark', 'ultra detailed', '고디테일', '자연스러운 분위기'. 사용자가 다른 비율(3:4, 9:16 등)을 명시했으면 그것으로 교체. 사용자가 명시적으로 '극사실' / 'photorealistic' / '8k'를 요청했을 때만 그 단어를 그대로 보존.",
  ...
  // ── EXAMPLE ────────────────────────────────────────────────────────────
  "Example shape (Korean) — input: '한국 20대 초반 미시, 예쁘고 귀여움. 캐주얼 여름 상의와 쇼츠. 배경 송도 코스트코. 헤어, 포즈, 배경 디테일, 조명, 표정 다 자유. 1:1.'\n\n[인물]\n  - 한국 20대 초반 미시, 예쁘고 귀여움\n\n[체형]\n  - 자연스러운 비율, 산뜻한 여름 분위기\n\n[의상]\n  - 캐주얼 여름 상의 (가벼운 소재, 깔끔한 핏)\n  - 캐주얼 쇼츠 (편안한 기장)\n\n[자유]\n  - 헤어스타일, 포즈, 배경, 조명, 표정 모두 자유\n  - 배경 큰 카테고리: 송도 코스트코\n\n[카메라]\n  - 아이폰 스냅 사진\n\n[퀄리티]\n  - 1:1 비율",
];
...
export function buildEnhancePayload(prompt, language, references = []) {
  ...
  const refNote = refs.length > 0
    ...
    : "";
  return {
    // Text-only rewrite (no tools) → rides the newest line. Override with
    // IMA2_TEXT_MODEL if the added latency becomes a problem.
    model: TEXT_MODEL,
    stream: true,
