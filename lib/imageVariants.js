...
export async function derivePreviews(rootDir, rel, { force = false, onError = null } = {}) {
  ...
  for (const v of VARIANTS) {
    ...
    try {
      ...
      const failure = { rel, suffix: v.suffix, message: err?.message || String(err) };
      if (typeof onError === "function") onError(failure);
      console.warn("[image-variants] derive failed:", rel, v.suffix, failure.message);
    }
  }
  return derivedCount;
}
...
export async function backfillPreviews(rootDir, options = {}) {
  ...
  async function worker(start) {
    for (let i = start; i < items.length; i += concurrency) {
      const rel = items[i];
      try {
        const created = await derivePreviews(rootDir, rel, {
          force,
          onError(error) {
            stats.ok = false;
            stats.failed++;
