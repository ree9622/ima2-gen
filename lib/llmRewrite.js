...
export function buildRewritePayload(prompt, { categories, refusalText, reasoningSummary } = {}) {
  return {
    // Text-only rewrite (no tools) → newest line, same latency as 5.5 on
    // this payload (measured 2026-08-03). See lib/models.js.
    model: TEXT_MODEL,
    stream: false,
 ...
 * @param {AbortSignal|null} [args.signal]  Optional cancellation signal.
...
export async function rewritePromptForSafety({
  ...
  let raw;
  try {
    raw = await runResponses({ url: oauthUrl, body, signal });
  } catch (e) {
    if (e?.code === "GENERATION_CANCELED" || e?.status === 499) throw e;
