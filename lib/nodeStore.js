...
import { derivePreviews } from "./imageVariants.js";
...
export async function saveNode(rootDir, { nodeId, b64, meta, ext = "png" }) {
  ...
  await atomicWriteJson(join(rootDir, DIR, filename + ".json"), meta, { spaces: 2 });
  await derivePreviews(rootDir, filename);
  return { filename };
}
...
export async function importExistingFile(rootDir, { sourceFilename, nodeId, meta }) {
  ...
  await atomicWriteJson(targetPath + ".json", meta, { spaces: 2 });
  await derivePreviews(rootDir, targetFilename);
