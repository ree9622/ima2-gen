...
// Rather than asking the client to chunk-pace the upstream — which
// makes the UI feel frozen during slow chunks — we throttle here so
// the client can fire all N prompts at once and we feed them to the
// proxy at a safe rate. Override with IMA2_MAX_CONCURRENT_OAUTH if
// the proxy gets faster.
//
...
const MAX_CONCURRENT_OAUTH =
  Math.max(1, Number(process.env.IMA2_MAX_CONCURRENT_OAUTH) || 4);
