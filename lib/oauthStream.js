...
// ──────────────────────────────────────────────────────────────────
// In-flight cap on concurrent OAuth-proxy calls.
//
// 2026-04-30: a 31-prompt batch fired all at once killed the local
// openai-oauth proxy (auto-restart loop) and on a separate run took
// down the main Node process with SIGSEGV (status=11), almost
// certainly from undici + better-sqlite3 contention under that load.
// User confirmed the historical "click generate one at a time, even
// rapidly" workflow always worked, so the safe baseline is strict
// serialization (concurrency 1). The client can fire all N prompts
// at once; the semaphore feeds them to the proxy one-by-one. This
// matches what manual button-mashing would do and is safe by
// definition. Override with IMA2_MAX_CONCURRENT_OAUTH if you've
// hardened the proxy and want to push more parallelism.
//
// The semaphore is process-global on purpose: a single proxy and a
// single GPU pool serve every caller (classic generate, node-mode
// generate, edit). They all queue through the same gate.
// ──────────────────────────────────────────────────────────────────
const MAX_CONCURRENT_OAUTH =
  Math.max(1, Number(process.env.IMA2_MAX_CONCURRENT_OAUTH) || 1);

let _oauthActive = 0;
const _oauthWaiters = [];

function _acquireOauthSlot() {
  if (_oauthActive < MAX_CONCURRENT_OAUTH) {
    _oauthActive += 1;
    return Promise.resolve();
  }
  return new Promise((resolve) => {
    _oauthWaiters.push(resolve);
  });
}

function _releaseOauthSlot() {
  const next = _oauthWaiters.shift();
  if (next) {
    // Hand the slot directly to the next waiter without bouncing the
    // counter — keeps active count bounded by MAX_CONCURRENT_OAUTH at
    // all times.
    next();
  } else {
    _oauthActive = Math.max(0, _oauthActive - 1);
  }
}

// Test-only: peek the current state. Not used at runtime.
export function _oauthSlotStats() {
  return { active: _oauthActive, waiting: _oauthWaiters.length, max: MAX_CONCURRENT_OAUTH };
}
...
export async function runResponses({ url, body, onPhase, onPartialImage }) {
  ...
  try {
    return await _runResponsesInner({ url, body, onPhase, onPartialImage });
  } finally {
    _releaseOauthSlot();
  }
}
