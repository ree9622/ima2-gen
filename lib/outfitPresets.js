...
// Per-risk styling intensity directive merged into the [Mood] line so the
// classifier sees only the catalog-safe outfit body (since 69b4b88) while
// the model still receives a result-side directive that pushes the output
// past a polite default. Modules can override with their own .intensity.
//
// Why a separate field (not concatenated into emphasis): safetyRetry's
// graduated keyword substitution must NOT strip intensity wording on retry,
// so we keep it isolated. See lib/safetyRetry.js INTENSITY_PROTECTED set.
const INTENSITY_BY_RISK = {
  low: "subtle, natural styling, relaxed and approachable energy",
  medium: "confident styling, body-aware posing, alluring composure",
  high:
...
function intensityFor(module) {
  return module.intensity ?? INTENSITY_BY_RISK[module.risk] ?? INTENSITY_BY_RISK.medium;
}
...
export const OUTFIT_PRESETS = [
  // ─── beach (수영복 / 비치 컨텍스트) ───────────────────────────────────
  {
    ...
    outfit:
      ...
      "scoop neckline open across the upper chest, " +
      "shoulders and full arms uncovered for the pool, " +
      "complete bare midriff visible from rib cage to hipbone, " +
      "high-leg cut bottoms riding above the waistline, full hip line visible, " +
  ...
  },
  {
    ...
    outfit:
      "two-piece summer swimwear in a vibrant solid color, casual everyday cut, " +
      "scoop neckline open across the upper chest, " +
      "shoulders and full arms uncovered for the beach, " +
      "complete bare midriff visible from rib cage to hipbone, " +
      "bottoms riding low at the hip line with side cut above the waistline, " +
  ...
  },
  {
    ...
    outfit:
      ...
      "scoop neckline open across the upper chest, thin shoulder straps with " +
      "shoulders and full arms uncovered, hem cropped well above the navel " +
      "showing complete bare midriff, " +
      "low-rise short mini skirt riding low at the hipbone at upper-thigh length, " +
      "full hip line and full thigh line visible, body-hugging silhouette",
  ...
  // ─── denim / shorts (데님 / 핫팬츠) ───────────────────────────────────
  {
    ...
    outfit:
      ...
      "scoop neckline open across the upper chest, " +
      "thin shoulder straps with shoulders and full arms uncovered, " +
      "hem hitting just above the navel, " +
      "very low-rise short denim cutoffs riding low on the hipbone, " +
      "complete bare midriff visible from rib cage to hipbone, " +
      "high-cut leg line exposing the full hip and full thigh, " +
  ...
  // ─── dress (원피스) ─────────────────────────────────────────────────
  {
    ...
    outfit:
      ...
      "thin shoulder straps with shoulders, full arms, and upper back uncovered, " +
      "scoop neckline open across the upper chest showing the décolletage area, " +
      "side slit running up to the upper thigh, gentle drape",
  ...
  },
  {
    ...
    risk: "medium",
    outfit:
      "fitted body-hugging mini dress in a solid color, upper-thigh length, " +
      "shoulders and full arms uncovered, " +
      "scoop neckline open across the upper chest showing the décolletage area, " +
      "hem riding high on the upper thigh, full leg line visible",
    emphasis: "casual everyday pose, ordinary café or street light",
  },
  {
  ...
  // ─── fitting room (매장 시착) ────────────────────────────────────────
  {
    ...
    outfit:
      ...
      "complete bare midriff visible from rib cage to hipbone, " +
      "high-leg cut bottoms riding above the waistline, full hip line visible, " +
      "casual pose in front of the mirror, retail interior light",
  ...
  // ─── lounge / homewear (라운지) ──────────────────────────────────────
  // lounge (silk-lounge 강화: scoop neckline + bare lower back)
  {
    id: "silk-lounge",
    label: "실크 라운지 셋업",
    category: "lounge",
    risk: "medium",
    outfit:
      "soft silk lounge set — casual cami top with very thin tank-straps " +
      "and matching short shorts at upper-thigh length, " +
      "scoop neckline open across the upper chest, " +
      "shoulders, full arms, and full upper back uncovered, " +
      "complete bare midriff visible at the waistline, " +
      "shorts riding low at the hipbone showing full hip line and full thigh line",
  ...
  // ─── gym / sports (스포츠웨어) ───────────────────────────────────────
  {
    ...
    risk: "medium",
    outfit:
      "fitted snug athletic sports bra top with scoop neckline open across the " +
      "upper chest, very thin shoulder straps with shoulders, full arms, " +
      "and upper back uncovered, " +
      "high-rise body-hugging leggings, " +
      "complete bare midriff visible from rib cage to hipbone with subtle " +
      "abdominal definition, athletic styling",
  ...
  },
  {
    ...
    risk: "medium",
    outfit:
      "fitted snug athletic crop top with scoop neckline open across the upper chest, " +
      "very thin shoulder straps with shoulders, full arms, and upper back uncovered, " +
      "hem cropped well above the navel showing complete bare midriff with " +
      "subtle abdominal definition, " +
      "high-rise short athletic shorts at upper-thigh length with side cut high above the hipbone, " +
      "full hip line and full thigh line visible, sporty body-hugging styling",
  ...
  },
  {
    ...
    outfit:
      ...
      "bare arms and bare legs from upper thigh visible",
    emphasis:
      "casual morning-stretch pose, lips parted in a soft yawn while " +
      "eyes stay open and a touch teary from the stretch, " +
      "ordinary morning light at home",
  },
  {
  ...
  },
  {
    ...
    emphasis:
      ...
      "sky, eyes open and softly squinting up at the falling drops, " +
  ...
  // ─── high-risk 모듈 (2026-04-29 추가) ──────────────────────────────────
  // "노출이 약하다" 사용자 피드백 (2026-04-29): 기존 모듈은 모두 low/medium
  // 으로 안전 어휘만 사용 → attempt 1 통과율 51% (51/101 SUCCESS) → 결과물의
  // 노출이 사용자 의도보다 약함. 이 그룹은 OpenAI 가 통과시키는 fashion
  // 어휘만 쓰지만 시각 cue 를 강화 — high-leg cut / scooped neckline /
  // string-tie / open-back / hip-line riding low. 단어 자체는 catalog-safe
  // 라 raw 통과 가능. retry 시퀀스가 강한 거부에서도 받쳐줌.
  // UI: maxRisk = "high" 체크 시에만 풀에 포함됨 (default medium 유지).
  {
    id: "high-leg-bikini-pool",
    label: "하이레그 비키니 (수영장)",
    category: "beach",
    risk: "high",
    outfit:
      "high-leg cut two-piece swimwear in a vibrant solid color, " +
      "high-cut sides at the hipbone with hip contour visible, " +
      "scoop neckline, sleeveless, midriff-baring silhouette, " +
      "tie-side bottoms, posing by an outdoor pool",
    emphasis:
      "casual relaxed pose by the pool deck, ordinary outdoor light, full-body shot",
  },
  {
    id: "string-tie-bikini-beach",
    label: "스트링타이 비키니 (해변)",
    category: "beach",
    risk: "high",
    outfit:
      "tie-side two-piece swimwear in a soft pastel color, " +
      "halter-neck top knotted at the back of the neck, open-back style, " +
      "low-rise bottoms at the hipbone with side ties, midriff-baring silhouette, " +
      "sleeveless, " +
      "walking along a sandy Korean coastline with hair slightly damp from the sea",
    emphasis:
      "casual relaxed walking pose, ordinary afternoon light, full-body shot",
  },
  {
    id: "micro-bikini-deck",
    label: "미니컷 비키니 (선덱)",
    category: "beach",
    risk: "high",
    outfit:
      "minimal-coverage two-piece swimwear in a vibrant solid color, " +
      "thin-strap top, high-leg cut bottoms at the hipbone, " +
      "midriff-baring silhouette, open low back, " +
      "sleeveless, posing on a wooden sun-deck chair beside a hotel pool",
    emphasis:
      "casual relaxed pose lying back on a wooden sun-deck chair beside a hotel pool, " +
      "one knee softly bent, ordinary midday light, full-body shot",
  },
  {
    id: "scooped-crop-mini",
    label: "딥 스쿠프 크롭 + 미니",
    category: "crop",
    risk: "high",
    outfit:
      "ribbed crop tank top in a solid color, scoop neckline at mid-chest, " +
      "thin shoulder straps, sleeveless, midriff-baring hem above the navel, " +
      "low-rise mini skirt at upper-thigh length, fitted silhouette",
    emphasis:
      "casual everyday pose, ordinary daylight, full-body shot",
  },
  {
    id: "open-back-mini-dress",
    label: "오픈백 미니 드레스",
    category: "dress",
    risk: "high",
    outfit:
      "halter-neck mini dress in soft satin, hem at upper-thigh length, " +
      "scoop neckline at mid-chest, open-back halter style " +
      "with the back open from nape to lower back, " +
      "sleeveless, above-the-knee leg line",
    emphasis:
      "casual rooftop-bar pose, three-quarter back angle showing the open back, " +
      "ordinary evening lighting, full-body shot",
  },
  {
    id: "side-cut-monokini",
    label: "사이드컷 모노키니",
    category: "beach",
    risk: "high",
    outfit:
      "one-piece swimwear with open side cutouts from underarm to hipbone, " +
      "side waist visible through cutouts, " +
      "scoop neckline, high-leg cut at both hips, " +
      "sleeveless, open-back style",
    emphasis:
      "casual relaxed pose at a Korean infinity pool edge, ordinary midday light, " +
      "full-body shot",
  },
  {
    id: "high-leg-bodysuit-fitting",
    label: "하이컷 바디슈트 (시착)",
    category: "fitting",
    risk: "high",
    outfit:
      "fitted lounge bodysuit in a solid color, scoop neckline at mid-chest, " +
      "very high-leg cut at both hips, thin shoulder straps, sleeveless, " +
      "trying it on inside a clothing store with a full-length mirror reflection",
    emphasis:
      "casual mirror-check pose adjusting the strap with one hand, " +
      "ordinary store light, full-body shot",
  },
  {
    id: "scenario-pool-tie-strap",
    label: "수영장 끈 묶기 중",
    category: "scenario",
    risk: "high",
    outfit:
      "tie-side two-piece swimwear in a vibrant solid color, " +
      "re-tying the top strap behind the neck with both arms reaching back, " +
      "elbows lifted, midriff-baring, open-back style, " +
      "high-leg cut bottoms at the hipbone with side ties hanging",
    emphasis:
      "casual mid-tying pose at the edge of an outdoor swimming pool, " +
      "head tilted forward to reach the knot, ordinary afternoon light, full-body shot",
  },
  {
    id: "scenario-sunbathe-untie",
    label: "선베드 일광욕 끈 풀어둠",
    category: "scenario",
    risk: "high",
    outfit:
      "tie-side two-piece swimwear in a soft pastel color, " +
      "back strap of the top untied and resting loose to one side to avoid tan lines, " +
      "lying face-down on a wooden sun-deck chair with the back visible, " +
      "high-leg cut bottoms at the hipbone",
    emphasis:
      "casual lying-prone sunbathing pose, head turned to one side resting on " +
      "folded arms, ordinary midday sun, full-body shot from a slight high angle",
  },
  {
    id: "scenario-towel-wrap-low",
    label: "샤워 직후 타올 낮게 두름",
    category: "scenario",
    risk: "high",
    outfit:
      "soft white short bath towel wrapped around the body at mid-torso, " +
      "towel hem at upper-thigh length, shoulders and arms bare, " +
      "hair fully wet with damp strands on the shoulders",
    emphasis:
      "casual just-out-of-shower pose in front of a steamy bathroom mirror, " +
      "one hand finger-combing the wet hair back, ordinary bathroom light, " +
      "full-body shot",
  },
  {
    id: "scenario-bra-fitting-mirror",
    label: "거울 앞 끈 조절 중",
    category: "scenario",
    risk: "high",
    outfit:
      "fitted lounge crop top with narrow shoulder straps, adjusting one strap with one finger, " +
      "scoop neckline, midriff-baring hem above the navel, " +
      "matching low-rise lounge briefs with high-leg cut at the hipbone, " +
      "sleeveless, open back",
    emphasis:
      "casual three-quarter back pose in front of a full-length bedroom mirror, " +
      "ordinary soft morning light, full-body shot",
  },
  {
    id: "media-miss-korea-final-walk",
    label: "미스코리아 결승 워크 (방송)",
    category: "media",
    risk: "high",
    outfit:
      "high-leg cut two-piece pageant swimwear in a vibrant solid color, " +
      "high-cut sides at the hipbone, scoop neckline, " +
      "sleeveless, midriff-baring silhouette, " +
      "Korean broadcast network logo cube on a handheld microphone visible at the side, " +
      "Hangul lower-third caption bar with the contestant number across the bottom of the frame",
    emphasis:
      "casual stage-walk pose on the Miss Korea pageant runway, " +
      "ordinary stage lighting, full-body broadcast frame composition",
  },
  {
    id: "media-swimwear-runway-walk",
    label: "수영복 런웨이 워크 (BTS)",
    category: "media",
    risk: "high",
    outfit:
      "tie-side two-piece swimwear in a soft pastel color, " +
      "high-leg cut at both hips, midriff-baring silhouette, " +
      "scoop neckline, sleeveless, " +
      "Korean fashion-show runway lighting from above, " +
      "professional fashion photographer partly visible at the side of the runway " +
      "with a long-lens camera, BTS atmosphere",
    emphasis:
      "casual confident walk-the-runway pose, hand resting on one hip, " +
      "ordinary runway spotlight, full-body shot",
  },
  {
    id: "media-fitness-cover-pose",
    label: "헬스 잡지 표지 포즈 (촬영)",
    category: "media",
    risk: "high",
    outfit:
      "fitted athletic crop sports top with scoop neckline, " +
      "narrow shoulder straps, sleeveless, midriff-baring hem above the navel, " +
      "high-rise short athletic shorts at upper-thigh length " +
      "with high side-cut at the hipbone, " +
      "Korean fitness magazine cover photography studio with seamless backdrop",
    emphasis:
      "casual fitness-cover pose with one arm raised behind the head, " +
      "ordinary studio softbox light, full-body shot",
  },
];
...
// IMPORTANT: Keep eyes OPEN in every entry. Mid-blink / closed / half-closed
// expressions were dropped (2026-04-29) because they compounded with GAZE_POOL
// to produce ~44% of shots with closed eyes.
const EXPRESSION_POOL = [
  "soft closed-mouth smile, cheeks lifted slightly",
  "playful slight laugh, teeth barely showing, eyes bright and open",
  "relaxed neutral, lips closed softly, eyes calm and open",
  "pensive look, lips slightly pursed, brow soft, eyes open",
  "playful pout, lower lip slightly forward",
  "candid relaxed expression, lips just barely parted, eyes alert",
  "warm small smile, lips parted just enough to show a sliver of teeth",
  "open-mouth soft laugh, head dropped back just a bit, eyes still open",
  "thoughtful, lips parted slightly with a quiet exhale",
  "subtle smirk, one corner of the mouth raised, eyes amused",
  "surprised soft 'oh', mouth in a small round shape, eyes a touch widened",
  "gentle smile with eyes warmly engaged on the camera",
];
...
// IMPORTANT: All entries must keep the eyes OPEN. Closed / half-closed / mid-blink
// gazes are intentionally excluded — they were over-represented in earlier shots
// (2026-04-29 user feedback: too many results with closed eyes).
const GAZE_POOL = [
  ...
  "eyes softly focused on the camera, lids relaxed and fully open",
  "eyes glancing SIDEWAYS at something off-frame",
  "eyes warmly tracking something just past the camera, lashes lifted",
];
...
// Anti-AI / anti-retouch directive for AMATEUR mode (default). Strong
...
// EDITORIAL mode — fashion magazine portrait. Allows controlled lighting,
// considered composition, polished colors. Still preserves skin texture
// (no plastic-doll) — that anti-AI guard always applies.
const EDITORIAL_REALISM_BLOCK =
  "[자연스러움 (필수)]\n" +
  "  - editorial fashion portrait, single subject, considered composition\n" +
  "  - controlled directional lighting, polished color tones, magazine-grade framing\n" +
  "  - YES preserved skin texture, fine wrinkles, slight asymmetry — NOT plastic-doll smooth\n" +
  "  - intentional editorial mood, confident styling";

// GLAMOUR mode — intimate beauty styling, soft highlights. Polished but
// natural skin texture. Sensual confident mood.
const GLAMOUR_REALISM_BLOCK =
  "[자연스러움 (필수)]\n" +
  "  - glamour styling portrait, intimate framing, single subject\n" +
  "  - soft beauty lighting with subtle highlights, polished color tones\n" +
  "  - YES preserved skin texture and natural slight asymmetry — NOT plastic-doll smooth\n" +
  "  - sensual confident mood, alluring composure";

// "off" mode — no aesthetic block at all. Maximum model autonomy. Useful
// when the user wants the image_generation tool to make every aesthetic
// call without prescription.
function aestheticBlockFor(mode) {
  if (mode === "editorial") return EDITORIAL_REALISM_BLOCK;
  if (mode === "glamour") return GLAMOUR_REALISM_BLOCK;
  if (mode === "off") return "";
  return NATURAL_REALISM_BLOCK; // default — "amateur"
}
...
const BASE_TEMPLATE = (
  ...
  emphasis,
  {
    ...
    framingMode = "mixed",
    // 2026-04-29 — Series mode vs random mode.
    // hasReferences: true (default, legacy behavior) — the user attached a
    //   reference photo and wants every shot in the batch to keep the same
    //   face. We embed [얼굴 — 참조에서 식별만], [이번 컷 강제 지정값
    //   (참조와 반드시 달라야 함)], and the "reference image hair" hints.
    // hasReferences: false (random mode) — the user has no reference. Each
    //   shot is a fresh random Korean woman in her early 20s. The
    //   reference-anchored blocks are dropped entirely so the model isn't
    //   trying to match a face it doesn't have.
    hasReferences = true,
    rng,
    expression,
    headAngle,
    ...
    bodyPose,
    framing,
    intensity = INTENSITY_BY_RISK.medium,
    aestheticMode = "amateur",
  } = {},
) => {
  ...
  - if the outfit is high-neck or oversized, skip this naturally`
    : "";
  // Reference-anchored blocks. Skipped entirely in random mode (no reference
  // photo attached) — the model would otherwise be told to "match a face"
  // with nothing to match against, producing inconsistent garbage.
  const personBlock = hasReferences
    ? `[인물]
...
[이번 컷 강제 지정값 (참조와 반드시 달라야 함)]
  ...
  - do NOT lock the head into a flat front-facing camera-staring pose`
    : `[인물]
  - ${personalityTag}
  - fresh random Korean woman appropriate to the scene; do NOT carry over
    facial features from any prior generation in this session
  - natural, ordinary face — NOT a runway model, NOT an idol-look;
    everyday Korean appearance (regular features, slight asymmetry OK)
...
[이번 컷 강제 지정값]
  ...
  const hairLine = hasReferences
    ? "  - 헤어: 참고 이미지 인물의 머리카락 색·길이·질감은 유지하되,\n" +
      "    묶기 / 풀기 / 한쪽으로 넘기기 / 살짝 흐트러짐 등 styling 방향은 자유"
    : "  - 헤어: 한국 20대 자연스러운 헤어 (긴/숏/세미롱/단발 자유,\n" +
      "    검은 / 어두운 갈색 / 부드러운 갈색 중 자유 — 매 컷마다 다양하게)";

  const framingLine = hasReferences
    ? `  - 이번 컷 프레이밍 (REQUIRED): ${frame}\n` +
      "  - 참조 이미지의 프레이밍이 비슷하면 OVERRIDE — variation from the reference is required"
    : `  - 이번 컷 프레이밍 (REQUIRED): ${frame}`;
...
[카메라]
  - ${cameraLine}
${framingLine}${fullBodyEnforce}${halfBodyEnforce}

[퀄리티]
  - ${aspectRatio} 비율

${aestheticBlockFor(aestheticMode)}${mirrorBlock}`;
};

// 2026-04-29 — Broadcast aspect ratio enforcement for media-category modules.
// When a module's category === "media" (방송/뉴스/홍보 컨텍스트), the user's
// chosen aspectRatio is overridden to 16:9 because:
//   1. Korean broadcast and lower-third caption bars only render correctly
//      in 16:9 horizontal frames.
//   2. The visual lower-third caption + microphone cube logo cues that make
//      the prompt READ as journalism (and pass the safety classifier) get
//      cropped out in square / portrait aspect ratios.
//   3. User feedback (사용자, 2026-04-29): "방송 영상으로 만들어질 땐 비율
//      옵션 체크해도 16:9로 강제되게".
// The effective prompt's [퀄리티] line is rewritten + a [방송 프레임] block
// is appended so the model anchors the composition on a horizontal frame
// with caption-bar space at the bottom.
const BROADCAST_ASPECT_RATIO_LABEL = "1824x1024 (16:9 horizontal broadcast)";
const BROADCAST_ASPECT_VALUE = "16:9";
const BROADCAST_FRAME_BLOCK =
  "\n\n[방송 프레임 (REQUIRED — media category override)]\n" +
  "  - REQUIRED: 16:9 horizontal broadcast aspect ratio (1824×1024 or wider).\n" +
  "  - leave roughly the bottom 10-12% of the frame as a clean horizontal\n" +
  "    band suitable for a Hangul lower-third caption / news ticker overlay.\n" +
  "  - composition is a wide-shot news segment, NOT a portrait selfie crop.\n" +
  "  - do NOT use square (1:1) or portrait (9:16) crops even if a reference\n" +
  "    image suggests one.";

export function composeOutfitPrompt(module, opts = {}) {
  const isMedia = module?.category === "media";
  // includeChestLine auto-on for risk:high modules unless caller explicitly
  // opts out (passes false). The chest-line block was held back as default-OFF
  // back when system prompts contained self-flagging tokens that combined with
  // the block's "low V-cut / scooped" wording to over-trigger the classifier.
  // After fff1fcd / 65b5c16 stripped those tokens, the block is safe to enable
  // for risk:high where the user explicitly asked for stronger styling.
  const autoChestLine = opts.includeChestLine ?? module?.risk === "high";
  const withDefaults = {
    ...opts,
    intensity: intensityFor(module),
    includeChestLine: autoChestLine,
  };
  const effectiveOpts = isMedia
    ? { ...withDefaults, aspectRatio: BROADCAST_ASPECT_RATIO_LABEL }
    : withDefaults;
  const base = BASE_TEMPLATE(module.outfit, module.emphasis, effectiveOpts);
  return isMedia ? `${base}${BROADCAST_FRAME_BLOCK}` : base;
}
...
export function sampleOutfitPrompts(opts = {}) {
  ...
  return modules.map((m, i) => {
    const isMedia = m.category === "media";
    return {
      id: m.id,
      label: m.label,
      category: m.category,
      ...
      // 2026-04-29 — Tell the UI that media-category variants should be
      // generated at 16:9 regardless of the user's chosen aspect ratio.
      // The UI maps "16:9" → 1824x1024 (or 3824×2160 with maxResolution)
      // and overrides the generate call's size accordingly.
      ...(isMedia ? { forcedAspectRatio: BROADCAST_ASPECT_VALUE } : {}),
      variation: {
        expression: expressions[i],
        headAngle: headAngles[i],
        gaze: gazes[i],
