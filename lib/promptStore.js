...
function rowToItem(row) {
  ...
  return {
    ...
    // fork extension: source/sourceUrl 추가 (NULL == 사용자 직접 작성)
...
export function createPrompt({ title = "", body, owner = null, source = null, sourceUrl = null }) {
  ...
  const db = getDb();
  db.prepare(`
    INSERT INTO prompts (id, title, body, pinned, use_count, last_used_at, created_at, updated_at, owner, source, source_url)
    VALUES (@id, @title, @body, 0, 0, NULL, @now, @now, @owner, @source, @sourceUrl)
  `).run({ id, title: trimmedTitle, body: trimmedBody, now, owner, source, sourceUrl });
