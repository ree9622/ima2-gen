...
const TRANSFORM_ONLY_HINTS = [
  ...
  /폰\s*(배경|바탕)\s*화면/,
  /월페이퍼/,
  /\battached\s+(photo|image)\b/i,
  /\breference\s+(photo|image)\b/i,
  /\bresolution\b/i,
  /\bresize\b/i,
  /\bwallpaper\b/i,
  /\bcrop\b/i,
...
function looksLikeTransformOnlyRequest(text) {
  if (!TRANSFORM_ONLY_HINTS.some((re) => re.test(text))) return false;
  return !HUMAN_VARIATION_HINTS.some((re) => re.test(text));
}
...
export function shouldBoostRefPrompt(prompt) {
  ...
  if (looksLikeTransformOnlyRequest(trimmed)) return false;
