...
// We learned this the hard way: the original "Sports broadcast … official
// press microphone with a network logo cube visible in frame" line caused the
// image model to literally render a press mic with a TV-station logo on
// almost every retry that landed on it. Keep frames abstract / journalistic
// in tone, not equipment-specific.
...
// 4 justification contexts instead of being stuck on (0, 1) forever. This
// prevents one "winning" prefix (historically the sports-broadcast frame)
// from dominating the output — when its visual prop bias survives a tone-
// down it produces a flood of look-alikes (e.g. broadcast-mic backdrops).
// Exposed via `_resetJustifyCycle()` so tests stay deterministic.
