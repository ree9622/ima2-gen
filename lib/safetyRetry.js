...
export function buildPromptAttempts(prompt, { hasRefs = false } = {}) {
  ...
  // Add the strong-English variant when prompt itself has a strong trigger,
  // OR when references are attached (the reference image may be the
  // trigger even though the prompt looks clean).
  if (isStrongTrigger(trimmed) || hasRefs) {
    // force: true lets the strong wrapper attach to clean prompts in
    // ref-mode even though no trigger keyword is present in the text.
