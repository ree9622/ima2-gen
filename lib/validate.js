...
const BACKGROUND = new Set(["auto", "opaque", "transparent"]);
...
export function validateBackground(v) {
  if (!BACKGROUND.has(v))
    return fail(
      "INVALID_BACKGROUND",
      `background must be one of ${Array.from(BACKGROUND).join(", ")}`,
    );
  return { ok: true, value: v };
}

export function validateCompression(v) {
  const n = Number(v);
  if (!Number.isInteger(n) || n < 0 || n > 100)
    return fail("INVALID_COMPRESSION", "compression must be an integer in [0, 100]");
